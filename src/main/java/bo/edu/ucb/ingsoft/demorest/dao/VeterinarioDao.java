    ...
    public Veterinario findVeterinarioById(Integer idVeterinario){
        ...
        try(Connection conn = dataSource.getConnection();
            PreparedStatement pstmt = conn.prepareStatement("select distinct " +
                    ...
                    "       v.nombre, " +
                    "       v.apellido , " +
                    "       (select avg(ca.calificacion) from CONSULTA co, CALIFICACION ca " +
                    "        WHERE co.id_veterinario = v.id_veterinario " +
                    "          and co.id_consulta= ca.id_consulta " +
                    "       ) estrellas, " +
                    "" +
                    "       e.especialidad, " +
                    "       v.departamento, " +
                    "       v.email, " +
                    "       vet.nombre Trabaja_con, " +
                    "       vet.direccion " +
                    "" +
                    "from veterinario v " +
                    "         left join veterinario_especialidad ve on v.id_veterinario = ve.id_veterinario " +
                    "         left join especialidad e on ve.id_especialidad = e.id_especialidad " +
                    "         left join veterinaria vet on v.id_veterinaria = vet.id_veterinaria " +
                    "where v.id_usuario = ?"))
        {
